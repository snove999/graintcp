...
export function updateBaselines({ log = console.log } = {}) {
  ...
  for (const rel of DOCS) {
    ...
    if (bl) text = text.slice(0, bl.start) + block + text.slice(bl.end);
    else { log(`[update] ${rel}: 未找到基准块，跳过（请先手工放置 <!-- doccheck:baseline ... -->）`); continue; }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      let s = ln;
      for (const e of edits) { s = s.slice(0, e.start) + e.text + s.slice(e.end); changes.push(`${rel}:${i + 1}  ${ln.slice(e.start, e.end)} → ${e.text}`); }
      lines[i] = s;
    }
    writeFileSync(join(ROOT, rel), lines.join('\n'));
  }
  log(`[update] 基准块已刷新；散文副本改写 ${changes.length} 处`);
...
if (isMain) {
  if (process.argv.includes('--update')) updateBaselines();
  const r = runDocCheck();
