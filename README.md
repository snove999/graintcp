...
1. **Workers & Pages → Create → Worker**，粘贴 `worker.js` 全文，Deploy。
...
完整 38 键以代码内 `getSafeEnv` 调用为准；D1 config 表中的键与环境变量同名，面板「保存配置」即写入。
...
`worker.obf.js` / `snippets.obf.js` 由 javascript-obfuscator（+terser 二次压缩）生成：保留顶层导出与全部行为（`renameGlobals` 关闭），本地标识符十六进制化/短名化 + 字符串数组化；worker 版另含 rc4 字符串编码与控制流平坦化。

- **`worker.obf.js`（818121B ≈ 799KB）——Workers 的部署件**：Dashboard 直接粘贴，或用 wrangler（`main` 已指向它）。Workers 无 32KB 限制，混淆不设防。
- **`snippets.obf.js`（39965B ≈ 39KB）——仅混淆参考，不可部署**：Snippets 平台 32KB 硬限额，而混淆变换实测膨胀 **+9532B（≈+9.3KB）**，物理上无法同时满足「混淆」与「≤32KB」（除非砍功能）。**Snippets 部署请粘贴 `snippets.js`**（30433B）——它本身已是无注释、单字符变量名的极限压缩形态，可读性等同混淆。
- ⚠️ **混淆产物字节不可复现**：构建器启用了 `controlFlowFlattening` / `stringArrayRotate` / `stringArrayShuffle`，带随机性，**同一份源码每次构建的字节与体积都会漂移**（实测三次：809008B → 817408B → 818121B）。因此**不能用哈希判断"产物与源码是否一致"**；唯一有效的门禁是**行为等价**——把产物复制成 `worker.js`/`snippets.js` 放进临时目录，跑同一套回归，**只允许「`snippets.obf.js` 体积 > 32768」这 1 项红**。**每次构建后都必须重跑。**
- 明文/混淆交叉验证：两份混淆产物均通过同一套 **151 项**回归。
...
离线桩环境（Node ≥ 24）跑 **151 项**回归：路径语法矩阵、addrParser、WS 中继流、xHTTP 双端全链路（含**下行数据回传**）、`/proxyip=` 7 种路径形态（含编码与尾随斜杠）、`XH_HS` 首包就绪边界、非法百分号编码健壮性、padding/TXT 池/测速拦截/UDP 拒绝、sstp/TURN 建连、订阅哨兵重建、转换器回源、D1 缓存与降频、getCustomIPs 并行、路由冒烟。

> 红绿对照：对修复前的代码（`git show HEAD:worker.js` / `git show HEAD:snippets.js`）跑同一套回归 → **35/151 失败**；修复后 **0/151**。这才是"修复真实生效"的证据链。
