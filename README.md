...
| `snippets.js` | Cloudflare **Snippets**（付费计划规则引擎） | 轻量版明文源码：受 32KB/5ms 限制，专注代理与订阅本体，无面板 |
| `worker.obf.js` / `snippets.obf.js` | 同上 | **混淆版（推荐部署用）**，与明文版行为一致（同一回归套件验证） |
| `wrangler.jsonc` / `schema.sql` | Workers | wrangler 部署配置 / D1 建表脚本 |
| `GrainTCP.js` | 参考 | 上游内核参考快照（部署文件内已内嵌，无需单独部署） |
| `test_harness.mjs` | 本地 | 90 项离线回归测试（Node ≥ 24） |
...
- **`worker.obf.js`（约 780KB）——推荐部署版**：Dashboard 直接粘贴，或用 wrangler（`main` 已指向它）。Workers 无 32KB 限制，混淆不设防。
- **`snippets.obf.js`（约 39KB）——仅混淆参考，不可部署**：Snippets 平台 32KB 硬限额，而混淆变换最小膨胀约 +5KB，物理上无法同时满足「混淆」与「≤32KB」（除非砍功能）。**Snippets 部署请粘贴 `snippets.js`**——它本身已是无注释、单字符变量名的极限压缩形态，可读性等同混淆。
- 明文/混淆交叉验证：两份混淆产物均通过同一套 90 项回归。
...
离线桩环境（Node ≥ 24）跑 **90 项**回归：路径语法矩阵、addrParser、WS 中继流、xHTTP 双端全链路、padding/TXT 池/测速拦截/UDP 拒绝、sstp/TURN 建连、订阅哨兵重建、转换器回源、D1 缓存与降频、getCustomIPs 并行、路由冒烟。
