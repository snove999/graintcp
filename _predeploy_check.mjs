...
if (!outSt) {
  ...
  lines.push(`[1/3] 产物与源码同源 ......... ${sameSource ? 'PASS' : 'FAIL'}`);
...
lines.push('');
if (!snipSt) {
  lines.push(`[2/3] 明文 snippets 体积 ....... FAIL`);
  ...
  lines.push(`[2/3] 明文 snippets 体积 ....... ${snipOk ? 'PASS' : 'FAIL'}`);
...
lines.push(`[3/3] 产物存在性（wrangler main）.. ${outExists ? 'PASS' : 'FAIL'}`);
...
// ── 总结论 ───────────────────────────────────────────────────────────
const pass = sameSource && snipOk && outExists;
lines.push('');
